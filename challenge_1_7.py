...
class AES():
	...
	def __shift_rows(self):
		"""
		Applies a left shift to state rows where the number of shifts is equal to
		the row number.
		"""
	...
	def __inv_shift_rows(self):
		"""
		Applies a right shift to state rows where the number of shifts is equal to
		the row number.
		"""
		self.state[0][1], self.state[1][1], self.state[2][1], self.state[3][1] = \
	...
	def __inv_mix_columns(self):
		"""
		Applies an inverse mixing function to the state columns. Uses preprocessing
		as described in Section 4.1.3 of The Design of Rijndael. The result is the
		same.
		"""
		for i in range(4):
